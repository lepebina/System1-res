...
public class CombinedStepsForStaticAnalysisThridPair {
    @Autowired
  ...
  OutGoingMessage mt = new OutGoingMessage();
    ...
    public void pairThreeScenarioOneWithoutRearrangement(String msisdn, String timeInHours, String message, String freeingTeaserId) throws Throwable {
    ...
    public void pairThreeScenarioTwoWithRearrangement(String msisdn, String tInHours, String msg, String freeTeaserId) throws Throwable {
        deleteEvent.setMsisdn(userAttributes.getMsisdn());
        deleteHandler.deleteCustomer();
        
        userAttributes.setMsisdn(msisdn);
        
        if (tInHours.equals("0")) {
            timeOffsetEvent.setOffSetInMillies(tInHours);
            timeOffsetHandler.sendDefaultApi();
        } else {
            long millis = Long.parseLong(tInHours) * 60 * 60 * 1000 - Long.parseLong(tInHours) * 60 * 60 * 1000 * 2;
            timeOffsetEvent.setOffSetInMillies(String.valueOf(millis));
            timeOffsetHandler.sendDefaultApi();
        }
        ...
        startEvent.setBulkId(freeTeaserId);
        startHandler.sendDefaultApi();
        //Thread.sleep(5000);
